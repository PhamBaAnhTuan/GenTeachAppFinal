...
// actions
import { getAction, signOutAction } from '@/src/store/actions';
...
export default function HomeScreen() {
  // auth
  const { user, accessToken, isAuthenticated, category, products, cart, video } = useAuth()
  // url
  ...
  const signOut = () => {
    ...
    dispatch(getAction('products', 'PRODUCT'))
    dispatch(getAction('carts', 'CART'))
  }
  ...
  const log = () => console.log(
    ...
    '\nURL: ', url,
    '\nVideo: ', video && video.map((vid: any) => vid.caption),
    '\nCategory: ', category && category.map((cat: any) => cat.name),
    '\nProducts: ', products && products.map((product: any) => product.name),
    '\nCart: ', cart && cart.map((item: any) => item.name),
  );
  // video
  const arr = [1, 2, 3]
      ...
      >
        {video && video.length > 0 ? (
          ...
          <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
            <Text style={{ fontSize: 15, fontWeight: 'bold', color: 'black' }}>No video</Text>
            <Button
              title='log'
              onPress={log}
            >
